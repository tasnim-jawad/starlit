...
export default {
  ...
  methods: {
    ...
    get_all_warehouses: async function () {
      let response = await axios.get("ware-houses?get_all=1");
    ...
    get_product_available_quantity: async function () {
      ...
      console.log(this.ware_house_id, this.product_id);

      if (this.ware_house_id && this.product_id) {
        let response = await axios.get(
          `get-warehouse-product-available-stock-quantity?warehouse_id=${this.ware_house_id}&product_id=${this.product_id}`
        );
        if (response.data.status == "success") {
          this.quantity_message = response.data.message;
        }
      }
    },
  },
