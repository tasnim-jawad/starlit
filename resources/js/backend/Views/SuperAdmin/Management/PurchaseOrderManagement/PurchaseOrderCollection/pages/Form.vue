...
export default {
  ...
  methods: {
    ...
    get_all_sales_orders: async function () {
      let response = await axios.get("purchase-orders?get_all=1");
    ...
    get_purchase_order_collection_history_by_purchase_order_id: async function (
      actionTitle,
      event,
      ref
    ) {
      ...
      let response = await axios.get("purchase-orders/" + event.target.value);
